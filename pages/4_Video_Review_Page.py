...
# File names
result_csv_name = 'review_results_klings_df_250203_part1.csv'


# Load data with caching
@st.cache_data
def load_data():
    full_df = pd.read_csv(result_csv_name)
...
def update_video():
    ...
    audio_url = df.iloc[st.session_state.current_index]['audio_url']
    st.empty()
...
st.caption(f"DemoID: {df.iloc[st.session_state.current_index]['sid']} Order: {df.iloc[st.session_state.current_index]['order']}")
